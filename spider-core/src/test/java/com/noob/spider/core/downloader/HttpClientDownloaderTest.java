...
import com.noob.spider.core.model.HttpRequestBody;
...
import com.noob.spider.core.proxy.SimpleProxyProvider;
...
public class HttpClientDownloaderTest {
    ...
    public void testDownloader() {
        com.noob.spider.core.downloader.HttpClientDownloader httpClientDownloader = new com.noob.spider.core.downloader.HttpClientDownloader();
    ...
    public void testGetHtmlCharset() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            private String getCharsetByUrl(String url) {
                com.noob.spider.core.downloader.HttpClientDownloader downloader = new com.noob.spider.core.downloader.HttpClientDownloader();
                Site site = Site.me();
                ...
                try {
                    httpResponse = httpClient.execute(new com.noob.spider.core.downloader.HttpUriRequestConverter().convert(requestGBK, site, null).getHttpUriRequest());
                } catch (IOException e) {
                    e.printStackTrace();
                }
                String charset = null;
    ...
    public void test_selectRequestMethod() throws Exception {
        ...
        server.get(eq(query("q"), "com/noob/spider")).response("get");
        server.post(eq(form("q"), "com/noob/spider")).response("post");
        server.put(eq(form("q"), "com/noob/spider")).response("put");
        server.delete(eq(query("q"), "com/noob/spider")).response("delete");
        server.request(and(by(method("HEAD")),eq(query("q"), "com/noob/spider"))).response(header("method","head"));
        server.request(and(by(method("TRACE")),eq(query("q"), "com/noob/spider"))).response("trace");
        final com.noob.spider.core.downloader.HttpUriRequestConverter httpUriRequestConverter = new HttpUriRequestConverter();
    ...
    public void test_download_when_task_is_null() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                final com.noob.spider.core.downloader.HttpClientDownloader httpClientDownloader = new com.noob.spider.core.downloader.HttpClientDownloader();
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423/");
                Page page = httpClientDownloader.download(request, Site.me().toTask());
    ...
    public void test_download_set_request_charset() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                final com.noob.spider.core.downloader.HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
                Request request = new Request();
