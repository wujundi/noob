...
import com.noob.spider.model.HttpRequestBody;
import com.noob.spider.proxy.Proxy;
import com.noob.spider.proxy.SimpleProxyProvider;
import com.noob.spider.selector.Html;
import com.noob.spider.utils.CharsetUtils;
...
public class HttpClientDownloaderTest {
    ...
    public void test_selectRequestMethod() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                request.setUrl("http://127.0.0.1:" + port + "/search?q=com.noob.spider.webmagic");
    ...
    public void test_set_request_cookie() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(eq(cookie("cookie"), "cookie-com.noob.spider.webmagic")).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
                ...
                request.addCookie("cookie","cookie-com.noob.spider.webmagic");
    ...
    public void test_disableCookieManagement() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(not(eq(cookie("cookie"), "cookie-com.noob.spider.webmagic"))).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
    ...
    public void test_set_request_header() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(eq(header("header"), "header-com.noob.spider.webmagic")).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                request.addHeader("header","header-com.noob.spider.webmagic");
                Page page = httpClientDownloader.download(request, Site.me().toTask());
                assertThat(page.getRawText()).isEqualTo("ok");
            }
        });
    ...
    public void test_set_site_header() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                Page page = httpClientDownloader.download(request, Site.me().addHeader("header","header-com.noob.spider.webmagic").toTask());
                assertThat(page.getRawText()).isEqualTo("ok");
            }
        });
    }
    ...
    public void test_set_site_cookie() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                Site site = Site.me().addCookie("cookie", "cookie-com.noob.spider.webmagic").setDomain("127.0.0.1");
